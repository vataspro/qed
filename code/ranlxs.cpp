...
// Push back n doubles at the end of a vector
...
void uranlxs_fill_complex_vector(std::vector<std::complex<double> >& r) {
  ...
  uranlxs_fill_vector(re);
  uranlxs_fill_vector(im);

  for (int k=0; k<=n; k++) {
    r[k] = std::complex<double>(re[k], im[k]);
  }

}

// Fill a vector with uniform random numbers in (-1, 1)
...
void rlxs_reset(int state[])
{
   ...
   for (k=0;k<12;++k) 
   {
      ...
      xflt[2*k]=(double)(sone_bit*y1);
      xflt[2*k+1]=(double)(sone_bit*y2);
   }
}
